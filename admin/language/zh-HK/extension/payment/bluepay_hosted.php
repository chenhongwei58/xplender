<?php
/**
 * description
 *
 * @copyright        2016 opencart.cn - All Rights Reserved
 * @link             http://www.guangdawangluo.com
 * @author           Sam Chen <sam.chen@opencart.cn>
 * @created          2016-10-22 09:12:56
 * @modified         2016-11-05 17:35:23
 */

// Heading
$_['heading_title']            = 'BluePay Hosted Form';

// Text
$_['text_extension']		 = '擴展';
$_['text_success']             = '成功: 您已修改了 BluePay 賬戶的詳細信息 ！';
$_['text_edit']                = '編輯 BluePay';
$_['text_bluepay_hosted'] = '<a href="http://www.bluepay.com/preferred-partner/opencart"target="_blank"> <img src="view/image/payment/bluepay.jpg"alt="BluePay Hosted Form"title="BluePay Hosted Form"style="border: 1px solid #EEEEEE;"/></a>';
$_['text_test']                = '測試模式';
$_['text_live']                = '在線模式(Live)';
$_['text_sale']                = '銷售模式';
$_['text_authenticate']        = '身份驗證';
$_['text_release_ok']          = '已成功發還';
$_['text_release_ok_order']    = '已成功發還';
$_['text_rebate_ok']           = '已成功回贈';
$_['text_rebate_ok_order']     = '已成功回贈，訂單狀態更新為已回贈';
$_['text_void_ok']             = '已成功取消訂單，訂單狀態更新為無效';
$_['text_payment_info']        = '支付信息';
$_['text_release_status']			= '付款已發還';
$_['text_void_status']				= '款項無效';
$_['text_rebate_status']			= '付款已回贈';
$_['text_order_ref']				= '訂單 ref';
$_['text_order_total']				= '授權總數';
$_['text_total_released']			= '發還總數';
$_['text_transactions']				= '交易記錄';
$_['text_column_amount']			= '金額';
$_['text_column_type']				= '類型';
$_['text_column_date_added']		= '加入日期';
$_['text_confirm_void']				= '你確定要取消款項？';
$_['text_confirm_release']			= '你確定你想要發還款項？';
$_['text_confirm_rebate']			= '你確定你想要回贈款項？';

// Entry
$_['entry_account_name']	 = 'Account Name';
$_['entry_account_id']		 = 'Account ID';
$_['entry_secret_key']		 = 'Secret Key';
$_['entry_test']			 = 'Transaction Mode';
$_['entry_transaction']		 = 'Transaction Method';
$_['entry_card_amex']		 = 'Amex';
$_['entry_card_discover']	 = 'Discover';
$_['entry_total']			 = 'Total';
$_['entry_order_status']	 = 'Order Status';
$_['entry_geo_zone']		 = 'Geo Zone';
$_['entry_status']			 = 'Status';
$_['entry_sort_order']		 = 'Sort Order';
$_['entry_debug']			 = 'Debug logging';

// Help
$_['help_total']					= '需要最低結賬金額才能啟動支付模組';
$_['help_debug']					= '啟用調試模式會將敏感數據寫到日誌文件。除非另有指示，您應該設定為停用。';
$_['help_transaction']				= '交易方法必須設置為付款以允許分期付款';
$_['help_cron_job_token']			= '請加長和難猜一點';
$_['help_cron_job_url']				= '設置一個例行性工作來調用此URL';

// Button
$_['button_release']		 = 'Release';
$_['button_rebate']			 = 'Rebate / refund';
$_['button_void']			 = 'Void';

// Error
$_['error_permission']				= '錯誤：您沒有權限修改 BluePay ！';
$_['error_account_name']			= '必須輸入賬戶名稱 ！';
$_['error_account_id']				= '必須輸入賬戶 ID ！';
$_['error_secret_key']				= '必須輸入秘密密鑰！';
